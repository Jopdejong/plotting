...
def webplot(input_dir, **kwargs):
	plot_dir = input_dir.split("www")
	if kwargs.get("nodate"):
		outputdir = plot_dir[-1]
	else:
		outputdir = str(datetime.date.today())+"/"+plot_dir[-1]
	print "\n\n", plot_dir
	if len(plot_dir)>2:
		print "found www more than twice in input dir"
	elif len(plot_dir) == 1:
		print "found no www in pathname"

	#datapath = "/data/atlas/users/jdegens/analysis/plotter/"
	plot_files, subdirs = get_inputs(input_dir, **kwargs)
	html_texts = {}
	for var in ["overview", "description", "subdir", "plot", "link"]:
		with open(os.path.expandvars("$PLOTTER_PATH/data/template_webplot_{}.html".format(var))) as htmlfile:
			html_texts[var] = string.Template(htmlfile.read())
	with open(os.path.expandvars("$PLOTTER_PATH/data/template_webplotting_{}.html".format("plot"))) as htmlfile:
		html_texts["plotjson"] = string.Template(htmlfile.read())

	# create remote dir
	if kwargs.get("copy"):
		print "Creating directory",input_dir,"..."
		mkdir_command = os.path.expandvars("$WEB_PLOTTING_MKDIR_COMMAND").format(subdir=outputdir)
		print mkdir_command
	...
	if kwargs.get("copy"):
		...
		for input_object in input_dir_content:
			...
			if os.path.isfile(full_input_object):
				copy_command = os.path.expandvars("$WEB_PLOTTING_COPY_COMMAND").format(source=full_input_object, subdir=outputdir)
				print "Copying",input_object, "with \n \t", copy_command
